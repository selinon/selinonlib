...
class TestMRU(object):
    ...
    def test_multiple_items(self):
        ...
        with pytest.raises(CacheMissError):
            ...
            cache.get(item_count - 1)

        for item_id in range(item_count - 2, -1, -1):
            assert cache.get(item_id) == self._item_id2item(item_id)

        assert cache.get(item_count) == self._item_id2item(item_count)

        # re-add 0
        cache.add(0, self._item_id2item(0))
    ...
    def test_multiple_items_get(self):
        ...
        for item_id in range(1, 6):
            cache.add(item_id*100, self._item_id2item(item_id*100))

        with pytest.raises(CacheMissError):
            # last item that was used
            cache.get(used_items[-1])

        for item_id in range(1, 5):
            with pytest.raises(CacheMissError):
                cache.get(item_id*100) == self._item_id2item(item_id*100)

        # last inserted when adding 5 dummy elements
        assert cache.get(500) == self._item_id2item(500)
